...
def f1_score(predictions, targets):
    f1 = metrics.f1_score(targets.argmax(axis=1).detach().cpu().numpy(), predictions[0].argmax(axis=1).detach().cpu().numpy())
    return torch.Tensor([f1])


def precision_score(predictions, targets):
    precision = metrics.precision_score(targets.argmax(axis=1).detach().cpu().numpy(), predictions[0].argmax(axis=1).detach().cpu().numpy())
    return torch.Tensor([precision])


def recall_score(predictions, targets):
    recall = metrics.recall_score(targets.argmax(axis=1).detach().cpu().numpy(), predictions[0].argmax(axis=1).detach().cpu().numpy())
    return torch.Tensor([recall])


def accuracy_score(predictions, targets):
    accuracy = metrics.accuracy_score(targets.argmax(axis=1).detach().cpu().numpy(), predictions[0].argmax(axis=1).detach().cpu().numpy())
