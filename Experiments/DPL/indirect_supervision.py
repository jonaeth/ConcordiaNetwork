...
class indirect_supervision:
    ...
    def construct_factor_graph(self):
        ...
        for idx, item in enumerate(self.instance['pos_neg_example']):
            ...
            self.g.factor(['z' + str(idx)],
                          potential=np.array(item[2]))  # probability 0, 1, initialized from initial probability

        # the DL factor
        for idx, item in enumerate(self.instance['pos_neg_example']):
            self.g.rv('DL' + str(idx), 2)
            self.g.factor(['DL' + str(idx)],
                          potential=np.array(item[2]))  # probability 0, 1, initialized from initial probability
        ...
        if self.opt['token_length']:
            ...
            for idx, item in enumerate(self.instance['pos_neg_example']):
                ...
                if len(mentions[idx]) >= 3:
                    self.g.factor(['token_length' + str(idx)],
                                  potential=np.array([1, 0, 0], dtype=np.float64))  # encoding the 0, 1
                    ...
                    self.g.factor(['token_length' + str(idx)],
                                  potential=np.array([0, 1, 0], dtype=np.float64))  # encoding the 0, 1
        ...
        if self.opt['str_length']:
            ...
            for idx, item in enumerate(self.instance['pos_neg_example']):
                ...
                if len(mention_str) >= 10:
                    self.g.factor(['str_length' + str(idx)],
                                  potential=np.array([1, 0, 0], dtype=np.float64))  # probability 0, 1
                    self.initial_potential['str_length' + str(idx)] = np.array([1, 0, 0], dtype=np.float64)
                    self.base_assignment['str_length' + str(idx)] = 0
                elif 10 > len(mention_str) >= 5:
                    self.g.factor(['str_length' + str(idx)],
                                  potential=np.array([0, 1, 0], dtype=np.float64))  # probability 0, 1
        ...
        if self.opt['postag']:
            ...
            for idx, item in enumerate(self.instance['pos_neg_example']):
                ...
                if ratio >= 0.5:
                    self.g.factor(['postag' + str(idx)],
                                  potential=np.array([1, 0], dtype=np.float64))  # probability 0, 1
                    self.initial_potential['postag' + str(idx)] = np.array([1, 0], dtype=np.float64)
                    self.base_assignment['postag' + str(idx)] = 0
                else:
                    self.g.factor(['postag' + str(idx)],
                                  potential=np.array([0, 1], dtype=np.float64))  # probability 0, 1
        ...
        if self.opt['special_token']:
            ...
            for idx, item in enumerate(self.instance['pos_neg_example']):
                ...
                if mention_str in ['spatial', 'insulin', ';', 'mother', 'brother', 'ghrelin', 'anova', 'lobe', 'star',
                                   'fish', 'leptin', 'hole', 'statin', 'pompe disease']:
                    self.g.factor(['special_token' + str(idx)],
                                  potential=np.array([1, 0], dtype=np.float64))  # probability 0, 1
        ...
        if self.opt['case_token']:
            ...
            for idx, item in enumerate(self.instance['pos_neg_example']):
                ...
                if ratio_token >= 0.4:
                    self.g.factor(['case_token' + str(idx)],
                                  potential=np.array([1, 0], dtype=np.float64))  # probability 0, 1
        ...
        if self.opt['case_str']:
            ...
            for idx, item in enumerate(self.instance['pos_neg_example']):
                ...
                if ratio >= 0.3:
                    self.g.factor(['case_str' + str(idx)],
                                  potential=np.array([1, 0], dtype=np.float64))  # probability 0, 1
        ...
        if self.opt['mention_ctx_entropy']:
            ...
            for idx, item in enumerate(self.instance['pos_neg_example']):
                ...
                if ratio >= 0.05:  # less important since too much entropy
                    ...
                    self.g.factor(['mention_ctx_entropy' + str(idx)],
                                  potential=np.array([1, 0, 0], dtype=np.float64))  # most important
        ...
        if self.opt['entity_entropy']:
            ...
            for idx, item in enumerate(self.instance['pos_neg_example']):
                ...
                if ratio >= 0.02:  # less important since too much entropy
                    ...
                    self.g.factor(['entity_entropy' + str(idx)],
                                  potential=np.array([1, 0, 0], dtype=np.float64))  # most important
        ...
        if self.opt['special_char']:
            ...
            for idx, item in enumerate(self.instance['pos_neg_example']):
                ...
                if '-' in mention or utils.hasNumbers(mention):  # important since contains special char
                    ...
                    self.g.factor(['special_char' + str(idx)],
                                  potential=np.array([0, 1], dtype=np.float64))  # most important
            ...
            for idx, item in enumerate(self.instance['pos_neg_example']):
                ...
                if '(' in left and ')' in right:
                    ...
                    if len(left_ctx.intersection(self.opt['entity_exclude_keys'][self.opt['entity_type']])) == 0:
                        self.g.factor(['acronym' + str(idx)],
                                      potential=np.array([1, 0], dtype=np.float64))  # it's the type we want
                ...
                # XXX ()
                    ...
                    if abs(len_acronym - len_text) <= 1 and len_acronym > 0:
                        self.g.rv('acronym' + str(idx), 2)
                        potential_type = True
                        if len(set(right_ctx).intersection(
                                self.opt['entity_exclude_keys'][self.opt['entity_type']])) == 0:
                            self.g.factor(['acronym' + str(idx)],
                                          potential=np.array([1, 0], dtype=np.float64))  # it's the type we want
                            self.initial_potential['acronym' + str(idx)] = np.array([1, 0], dtype=np.float64)
                            self.base_assignment['acronym' + str(idx)] = 0
        ...
        if self.opt['cross_sentence']:
            ...
            for idx, item in enumerate(self.acronym_list):
                ...
                if item[1]:
                    self.g.factor(['cross_sentence' + str(idx)],
                                  potential=np.array([1, 0], dtype=np.float64))  # probability 0, 1
                ...
                for idx2 in range(len(self.instance['pos_neg_example'])):
                    if mentions[idx1] == mentions[idx2] and idx1 != idx2:
                        ...
                        if item[0] < item[1]:
                            ...
                            self.g.factor(['DL' + str(idx2)], potential=np.array(new_prob, dtype=np.float64))
            ...
            for idx1, item in enumerate(self.acronym_list):
                for idx2 in range(len(self.instance['pos_neg_example'])):
                    if item[0] == mentions[idx2]:
                        ...
                        self.current_pairwisepotential[
                            'cross_sentence' + str(idx1) + '&&' + 'z' + str(idx2)] = np.array([[0.9, 0.1],
                                                                                               [0.1, 0.9]
                                                                                               ])
    ...
    def get_current_parameters(self):
        ...
        for pairwise_keys in self.current_pairwisepotential.keys():
            key1 = pairwise_keys.split('&&')[0]
            key2 = pairwise_keys.split('&&')[1]
            para_key = key1.translate(str.maketrans('', '', digits)) + "&&" + key2.translate(
    ...
    def count(self, assignment, prob):
        ...
        for pairwise_key in self.current_pairwisepotential.keys():
            ...
            key2 = pairwise_key[1]
            v1 = assignment[key1]
            key1 = key1.translate(str.maketrans('', '', digits))
            v2 = assignment[key2]
            key2 = key2.translate(str.maketrans('', '', digits))
            para_key = key1 + "&&" + key2
