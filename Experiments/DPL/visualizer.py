    ...
    def write_context(out_file, context, matches, class_entity, example_index):
        ...
        for idx, word in enumerate(context):
            ...
            if current_class and not other_class:
                out_file.write('<u>')
                out_file.write(
                    '<mark class="g%(class)s" cursor="pointer" title="%(pointer_text)s" style="background-color:%(rgb)s;">%(word)s </mark>' %
                    {'pointer_text': class_entity + ":" + str(pred), 'rgb': attention_to_rgb(0.5), 'word': word,
                     'class': str(example_index) + "-" + word.replace("@", "")})
                out_file.write('</u>')
            elif current_class and other_class:
                out_file.write('<u>')
                out_file.write(
                    '<mark class="g%(class)s" cursor="pointer" title="%(pointer_text)s" style="background-color:%(rgb)s;">%(word)s </mark>' %
                    {'pointer_text': class_entity + category + str(pred), 'rgb': attention_to_rgb(1), 'word': word,
                     'class': str(example_index) + "-" + word.replace("@", "")})
                out_file.write('</u>')
            else:
                out_file.write(
                    '<mark class="g%(class)s" cursor="pointer" title="%(pointer_text)s" style="background-color:%(rgb)s;">%(word)s </mark>' %
                    {'pointer_text': "no matching", 'rgb': attention_to_rgb(0), 'word': word,
                     'class': str(example_index) + "-" + word.replace("@", "")})
...
def make_html_file_confidence(examples, filename, gene_key):
    ...
    def write_context(out_file, context, matches, example_index):
        ...
        for idx, word in enumerate(context):
            ...
            # checking whether they overlapping or not, only displace the class_entity
            if " ".join([i.decode("utf-8") for i in context[matches[0]:matches[1] + 1]]) in gene_key.keys():
                id = gene_key[" ".join([i.decode("utf-8") for i in context[matches[0]:matches[1] + 1]])]
            else:
                id = ''
            if current_class:
                out_file.write('<u>')
                out_file.write(
                    '<mark class="g%(class)s" cursor="pointer" title="%(pointer_text)s" style="background-color:%(rgb)s;">%(word)s </mark>' %
                    {'pointer_text': id, 'rgb': attention_to_rgb(1), 'word': word,
                     'class': str(example_index) + "-" + word.replace("@", "")})
                out_file.write('</u>')
            else:
                out_file.write(
